...
ac(checks, 'P8', 'Pruebas criticas de humo', 'critical', () => {
  if (!runChecks) return { status: 'WARN', details: 'No se ejecutaron comandos (--no-commands).', evidence: ['npm run test:smoke'], ownerAction: 'Ejecutar test:smoke.' };
...
ac(checks, 'P9', 'Build de backend y arranque', 'critical', () => {
  if (!runChecks) return { status: 'WARN', details: 'No se ejecutaron comandos (--no-commands).', evidence: ['npm run build:server'], ownerAction: 'Ejecutar build:server.' };
