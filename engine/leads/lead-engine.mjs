/**
 * Lead Engine — engine/leads/lead-engine.mjs
 * Pure business logic: SQLite database operations, seed loading,
 * OpenStreetMap enrichment, CSV export, dashboard updates.
...
async function getDB(dbBuf) {
  ...
  const db = new SQL.Database(dbBuf || null);
  db.run('PRAGMA foreign_keys=ON');
  return db;
}

export async function runLeadEngine(args = {}) {
  const DB_PATH = args.dbPath || path.resolve('ops/database/leads.db');
  const SEED_PATH = args.seedPath || path.resolve('ops/database/seed-companies.json');
  const EXPORT_DIR = args.exportDir || path.resolve('ops/database/exports');
  const DASH_PATH = args.dashPath || path.resolve('ops/runtime/dashboard-unified.json');
  const seedDataOverride = args.seedData || null;
  ...
  if (mode === 'seed' || (mode === 'auto' && beforeCount === 0)) {
    console.log(`\n=== LOADING SEED DATA ===`);
    const seed = loadJSON(SEED_PATH) || seedDataOverride || SEED_DATA;
    let loaded = 0;
    ...
    console.log(`Source: ${seed.source || SEED_DATA.source}`);
  }
...
`);
  ...
  return { finalCount, newCompanies, exportDir: EXPORT_DIR };
}
