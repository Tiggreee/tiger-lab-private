...
function KpiBlock({ label, value }: { label: string; value: string | number }) {
  return (
    <div className="bg-gray-100 rounded-lg p-6 text-center text-2xl font-bold">
      {label}: <span className="block text-3xl">{value}</span>
    </div>
  )
}
...
export default function Dashboard({ status = 'idle', metrics, onActivateMonetization }: DashboardProps) {
  ...
  return (
    <div className="min-h-screen bg-white p-8">
      <h2 className="text-2xl font-bold mb-8">Tu sistema esta vivo</h2>
